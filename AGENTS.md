...
- 默认只输出关键状态日志；如果启用 `-v` 或 `--verbose`，再额外打印策略、识别和定位相关的详细日志。文案尽量短，优先输出“当前判断方式”和“当前阶段结果”。
...
- 当前仓库不再维护 MCP server，对外入口只保留 skill 文档和 `scripts/unity-auto-play.py` 脚本。
- 如果后续重新引入新的调用入口，优先复用现有脚本或明确的工作流 helper，不要复制一套独立的 Unity UI 自动化实现。
- 如果调用方式、能力边界或使用说明发生变化，必须同时更新 `README.md`、`AGENTS.md` 和 `SKILL.md`。
